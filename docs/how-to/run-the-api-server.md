...
The typical client flow — create a conversation, send a message, read the event
stream:
...
# send a message; the response is newline-delimited JSON events
curl -N -X POST "http://127.0.0.1:8000/conversation/$SID?content=enumerate%20the%20target&mode=supervised"
```
